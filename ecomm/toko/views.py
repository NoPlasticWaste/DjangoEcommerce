...
class HomeListView(generic.ListView):
    ...
    def get_queryset(self):
        ...
        if category:
            queryset = ProdukItem.objects.filter(kategori=switcher.get(category))
        else:
    ...
    def get_context_data(self, **kwargs):
        ...
        context['PILIHAN_KATEGORI'] = PILIHAN_KATEGORI
        return context
...
def remove_single_item_order_summary(request, slug):
    produk_item = get_object_or_404(ProdukItem, slug=slug)
    order_query = Order.objects.filter(
        user=request.user, ordered=False
    )
...
def add_item_order_summary(request, slug):
    if request.user.is_authenticated:
        produk_item = get_object_or_404(ProdukItem, slug=slug)
        order_produk_item, _ = OrderProdukItem.objects.get_or_create(
            produk_item=produk_item,
